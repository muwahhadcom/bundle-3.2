...
export function DataTable<T>({
  ...
  const isActionPermission = hasAnyPermission(actionPermission || []);
  ...
  const currentPageIds = data
    ?.map?.((item) => (getRowId ? getRowId(item) : (item as any)._id))
  ...
  const getSortIcon = (column: (typeof columns)[0]) => {
    ...
    if (effectiveSortBy !== key)
      return <ChevronsUpDown size={14} className="text-slate-400 shrink-0" />;
        ...
        <TableHeader className="bg-gray-50 dark:bg-(--card-color) border-b border-slate-200/60 dark:border-(--card-border-color)">
          <TableRow className="hover:bg-transparent border-none">
            ...
            {columns.map((column, index) => {
              ...
              if (key === "Actions" && !isActionPermission) {
                return null;
              }
          ...
          {isLoading && data.length === 0 ? (
              ...
              <TableRow key={rowIndex}>
                ...
                {columns.map((_, colIndex) => (
                  <TableCell key={colIndex} className="px-6 py-5">
                    <Skeleton
                      className={cn("h-4", colIndex === 0 ? "w-2/3" : "w-1/2")}
                    />
                  </TableCell>
                ))}
              </TableRow>
            ...
            data?.map?.((item, rowIndex) => {
              const rowId = getRowId
                ? getRowId(item)
                : (item as any)._id || (item as any).id || rowIndex.toString();
              ...
              return (
                ...
                >
                  ...
                  {columns.map((column, colIndex) => {
                    if (column.header === "Actions" && !isActionPermission) {
                      return null;
                    }
                    const cellContent = column.cell
                      ...
                      : column.accessorKey
                        ...
                        : null;
                    const copyValue = column.copyField
                      ? String(item[column.copyField])
                      : column.accessorKey
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-2.5">
                          <div className="text-sm font-medium text-slate-600 dark:text-slate-300 group-hover/row:text-slate-900 dark:group-hover/row:text-white transition-colors ">
                            {cellContent}
                          </div>
                          {column.copyable && copyValue && (
                            <Button
                              onClick={(e) =>
                                !is_demo_mode && handleCopy(e, copyValue)
                              }
