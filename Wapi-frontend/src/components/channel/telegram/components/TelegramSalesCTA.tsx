...
import { defaultTelegramSales } from "@/src/data/ChannelConnectionInfo";
import { Button } from "@/src/elements/ui/button";
import { TelegramSalesCTAProps } from "@/src/types/channel";
import { Check } from "lucide-react";

export default function TelegramSalesCTA({
  sales,
  isAuthenticated,
  router,
  primaryColor,
  tgGradient,
}: TelegramSalesCTAProps) {
  const title = sales.title || defaultTelegramSales.title;
  const subtitle = sales.subtitle || defaultTelegramSales.subtitle;
  const button1_title =
    sales.button1_title || defaultTelegramSales.button1_title;
  const button1_url = sales.button1_url || defaultTelegramSales.button1_url;
  const button2_title =
    sales.button2_title || defaultTelegramSales.button2_title;
  const button2_url =
    sales.button2_url ||
    sales.button2_description ||
    defaultTelegramSales.button2_url;
  const bullets = Array.isArray(sales.bullets)
    ? sales.bullets
    : defaultTelegramSales.bullets;
    ...
    <section className="py-[calc(20px+(50-20)*((100vw-320px)/(1920-320)))] bg-white">
      <div className="container mx-auto px-[calc(8px+(24-8)*((100vw-320px)/(1920-320)))] md:px-12 xl:px-16">
        <div className="max-w-6xl mx-auto rounded-2xl bg-gradient-to-br from-slate-950 via-slate-900 to-sky-950 p-[calc(10px+(30-10)*((100vw-320px)/(1920-320)))] text-white text-center relative overflow-hidden shadow-2xl shadow-sky-950/20">
          <div
            className="absolute top-[-50%] left-[-50%] w-[100%] h-[100%] rounded-full bg-[#0088cc]/15 blur-[130px] pointer-events-none"
