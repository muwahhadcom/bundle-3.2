...
import { REPLY_TONES } from "@/src/data/components";
...
const SuggestReplyModal: React.FC<SuggestReplyModalProps> = ({
  isOpen,
  onClose,
  lastMessages,
  onUseReply,
        ...
        <DialogHeader className="sm:p-7 p-4 pb-2! shrink-0">
          <div className="flex items-center gap-4">
            <div className="relative">
              ...
              <div className="absolute -top-1 -right-1">
                <motion.div
                  ...
                  className="w-6 h-6 rounded-full bg-primary flex items-center justify-center text-white shadow-lg"
                >
                  <Sparkles size={12} />
                </motion.div>
              </div>
            ...
            <div className="text-left rtl:text-right">
              ...
              <DialogDescription className="text-slate-500 dark:text-gray-500">
                Craft the perfect response with AI powered suggestions.
              </DialogDescription>
            </div>
            <Button
              variant="ghost"
              size="icon"
              onClick={onClose}
              className="rounded-lg ml-auto rtl:ml-[unset] rtl:mr-auto dark:hover:bg-(--table-hover)"
            >
              <X size={20} />
            </Button>
          </div>
                    ...
                    <div
                      className={cn(
                        ...
                        isSelected
                          ...
                          : "bg-white dark:bg-(--card-color) text-slate-500",
                      )}
                    >
                      <Icon size={20} />
                    </div>
                    ...
                    >
                      ...
                      <div className="flex items-center justify-end gap-3 transition-opacity">
                        <Button
                          ...
                          className="h-9 px-3 rounded-lg text-slate-500 hover:bg-slate-100 dark:hover:bg-(--table-hover) hover:text-primary"
                        >
                          {copiedIndex === index ? (
              ...
              !isLoading && (
                <div className="py-8 flex flex-col items-center justify-center text-center space-y-3 border-2 border-dashed border-slate-100 dark:border-(--card-border-color) rounded-lg opacity-50">
                  ...
                  <p className="text-sm text-slate-500 dark:text-gray-400 font-medium">
                    Click generate to see magic suggestions
                  </p>
                </div>
              )
            )}
        ...
        <DialogFooter className="bg-slate-50/50 dark:bg-(--card-color) p-4 border-t border-slate-100 dark:border-(--card-border-color) shrink-0">
          <div className="w-full flex items-center justify-center gap-2 text-slate-400">
            ...
            <p className="text-[10px] font-medium uppercase tracking-widest text-center">
              Suggestions based on {lastMessages?.length || 0} conversation
              context
            </p>
          </div>
        </DialogFooter>
