  ...
  return (
    <div className="mx-auto space-y-8 animate-in slide-in-from-right-4 duration-300">
      <div className="space-y-2 relative">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2 text-primary font-semibold text-lg">
            <Bot size={22} />
            <span>{t("ai_intelligence_title", "AI Intelligence (Gemini)")}</span>
          </div>
          <InfoModal dataKey="ai_intelligence" iconSize={22} className="text-gray-400 hover:text-primary transition-colors" />
        </div>
        <p className="text-sm text-muted-foreground">{t("configure_brain_desc", "Configure the brain of your assistant using Google's powerful Gemini models.")}</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div className="space-y-2">
          <Label className="text-sm font-medium">{t("model_engine", "Model Engine")}</Label>
          <div className="p-3 bg-slate-50 dark:bg-(--page-body-bg) rounded-lg border border-slate-100 dark:border-(--card-border-color) flex items-center justify-between">
            ...
            <Badge variant="outline" className="text-[10px] bg-primary/5 text-primary border-primary/20 uppercase tracking-tighter">
              {t("fast_lightning", "Fast Lightning")}
            </Badge>
          </div>
        </div>
        <div className="space-y-2">
          <Label className="text-sm font-medium">{t("ai_api_key", "AI API Key")}</Label>
      ...
      <div className="space-y-4">
        <div className="space-y-2">
          <Label className="text-sm font-medium">{t("system_instructions_personality", "System Instructions / Personality")}</Label>
          <Textarea placeholder={t("system_instructions_placeholder", "You are a helpful customer service representative for a luxury hotel...")} {...getFieldProps("ai_config.prompt")} className="min-h-40 rounded-lg p-4 shadow-inner" />
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-2">
            <Label className="text-sm font-medium text-slate-500 flex items-center gap-1.5">
              <Globe size={14} /> {t("knowledge_base_url", "Knowledge Base URL")}
            </Label>
            <Input placeholder={t("knowledge_base_url_placeholder", "https://docs.yourcompany.com")} {...getFieldProps("ai_config.training_url")} className="rounded-lg h-12" />
          </div>
          <div className="flex items-center justify-between p-4 bg-slate-50 dark:bg-(--page-body-bg) rounded-lg border border-slate-100 dark:border-(--card-border-color)">
            <div className="space-y-0.5">
              <span className="text-sm font-semibold">{t("natural_conciseness", "Natural Conciseness")}</span>
              <p className="text-[10px] text-slate-500 leading-tight">{t("optimizes_ai_vocal_naturalness", "Optimizes AI for vocal naturalness.")}</p>
            </div>
