...
import { useGetUserProductsQuery } from "@/src/redux/api/catalogueApi";

export function SendCatalogNode({ data, id }: any) {
  const { setNodes } = useReactFlow();
  const [touched, setTouched] = useState(false);
  const { data: resData, isLoading: loading } = useGetUserProductsQuery({ limit: 100, type: "whatsapp" });
  ...
  return (
    ...
    >
      <div className="space-y-4">
        ...
        >
          <Input
            placeholder="e.g. Send Catalog Product"
            value={data.name || ""}
