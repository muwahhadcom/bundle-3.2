...
import { customIcon } from "@/src/data/chat";
      ...
      <DialogContent className="max-w-3xl w-[calc(100vw-2rem)] sm:w-[calc(100vw-4rem)] md:w-full p-0! gap-0 overflow-hidden bg-white dark:bg-(--card-color) border-none shadow-2xl rounded-lg max-h-[90vh] sm:max-h-[85vh] flex flex-col">
        <DialogHeader className="p-4 sm:p-5 pb-0 shrink-0">
          <DialogTitle className="flex items-center gap-2 text-lg sm:text-xl font-bold text-primary">
            ...
            <Button
              onClick={handleClose}
              className="p-1 bg-gray-100 hover:bg-slate-200 dark:bg-transparent dark:hover:bg-(--table-hover) rounded-lg transition-colors shrink-0"
            >
        ...
        <div className="p-4 sm:p-6 pt-0! space-y-3 sm:space-y-4 overflow-y-auto flex-1 custom-scrollbar">
          ...
          <div className="relative h-75 sm:h-87.5 md:h-100 w-full rounded-lg overflow-hidden border border-slate-100 dark:border-(--card-border-color) shadow-inner group">
            ...
            <div className="absolute top-3 sm:top-4 right-3 sm:right-4 z-1000 flex flex-col gap-2">
              <Button
                ...
                className="h-9 w-9 sm:h-10 sm:w-10 text-slate-500 hover:text-white rounded-lg shadow-lg transition-all active:scale-95 bg-white dark:bg-(--card-color) dark:text-neutral-300"
                disabled={isLocating}
              >
                {isLocating ? (
            ...
            <div className="flex items-start gap-2 sm:gap-3">
              ...
              <div className="flex-1 space-y-1 min-w-0">
                ...
                {isReverseGeocoding ? (
                  ...
                  <>
                    <p className="text-xs sm:text-sm font-semibold text-slate-700 dark:text-neutral-200 leading-relaxed wrap-break-word">
                      {address || t("select_location_on_map")}
                    </p>
          ...
          <div className="flex items-center gap-2 sm:gap-3 pt-1 sm:pt-2">
            <Button
              variant="ghost"
              onClick={onClose}
              className="flex-1 h-10 sm:h-12 dark:bg-(--page-body-bg) text-sm sm:text-base rounded-lg px-3 sm:px-4 py-4 sm:py-5 font-bold dark:hover:bg-(--table-hover) hover:bg-slate-100 transition-all"
            >
              {t("cancel")}
            </Button>
            <Button
