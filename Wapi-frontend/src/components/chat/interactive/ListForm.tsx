...
const ListForm = forwardRef<ListFormHandle, ListFormProps>(
  ({ onSend, setIsSending }, ref) => {
    ...
    useImperativeHandle(ref, () => ({
      submit: async () => {
        ...
        if (
          listSections.some(
            (s) => !s.title.trim() || s.items.some((i) => !i.title.trim()),
          )
        ) {
    ...
    const handleAddSection = () => {
      if (listSections.length >= 10) {
        toast.error("Maximum 10 sections are allowed");
        return;
      }
      setListSections([
        ...listSections,
    ...
    const handleAddItem = (sectionIndex: number) => {
      ...
      if (section.items.length >= 10) {
        toast.error("Maximum 10 rows are allowed per section");
        return;
      }
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <Label className="text-sm font-semibold text-slate-700 dark:text-slate-300">
              Sections & Items ({listSections.length}/10)
            </Label>
            <Button
          ...
          <div className="space-y-6">
            {listSections.map((section, sIndex) => (
              <div
                key={section.id}
                className="p-4 rounded-lg border border-slate-200 dark:border-(--card-border-color) bg-slate-50/50 dark:bg-(--card-color) space-y-4"
              >
                ...
                <div className="pl-9 space-y-3 custom-scrollbar overflow-y-auto max-h-97">
                  {section.items.map((item, iIndex) => (
                    <div
                      key={item.id}
                      className="relative p-3 rounded-lg bg-white dark:bg-(--card-color) border border-slate-100 dark:border-(--card-border-color) space-y-2 shadow-sm group"
                    >
                      <div className="flex items-center justify-between">
                        ...
                        {section.items.length > 1 && (
                          <Button
                            ...
                            className="text-rose-400! hover:text-rose-500! bg-[unset]! transition-colors"
                          >
                            <Trash2 size={14} />
                          </Button>
                        )}
                  ...
                  >
                    <Plus size={14} /> Add Row ({section.items.length}/10)
                  </Button>
                </div>
              </div>
            ))}
