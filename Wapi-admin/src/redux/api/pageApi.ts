import {  GetPagesParams } from "@/src/types/store";
import { baseApi } from "./baseApi";
import { GetPagesResponse, Page } from "@/src/types/landingPage";
...
export const pageApi = baseApi.enhanceEndpoints({ addTagTypes: ["Page"] }).injectEndpoints({
  endpoints: (builder) => ({
    ...
    uploadPageMedia: builder.mutation<{ success: boolean; fileUrl: string }, FormData>({
      query: (formData) => ({
        url: "/pages/upload-media",
        method: "POST",
        body: formData,
      }),
    }),
...
export const { 
  ...
  useTogglePageStatusMutation, 
  useDeletePagesMutation,
  useUploadPageMediaMutation 
} = pageApi;
