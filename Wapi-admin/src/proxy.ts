...
export function proxy(request: NextRequest) {
  ...
  if (!token && !isLoginPage && !isForgotPasswordFlow && !isPublicPage) {
    return NextResponse.redirect(new URL(getUrlWithBasePath("/auth/login"), request.url));
  }

  if (token && isLoginPage) {
    return NextResponse.redirect(new URL(getUrlWithBasePath("/dashboard"), request.url));
  }
