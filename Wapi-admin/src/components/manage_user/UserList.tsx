...
const UserList = ({
  ...
  const [isResetLimitsConfirmOpen, setIsResetLimitsConfirmOpen] =
    useState(false);
  const [isDeleteConfirmOpen, setIsDeleteConfirmOpen] = useState(false);
  const [impersonatingId, setImpersonatingId] = useState<string | null>(null);
  const [startImpersonation] = useStartImpersonationMutation();
  const [sendResetLink, { isLoading: isSendingReset }] =
    useSendResetPasswordLinkMutation();
  const [resetLimits, { isLoading: isResettingLimits }] =
    useResetSubscriptionLimitsMutation();
  ...
  const allColumns: (ColumnDef<User> & { id: string })[] = [
    ...
    },
    {
      ...
      accessor: (user) => (
        <Badge
          className={`border-none text-xs font-medium ${user.status ? "bg-(--light-primary) hover:bg-(--light-primary) text-primary dark:bg-emerald-900/20 dark:text-primary" : "bg-red-100 text-red-700 dark:bg-red-900/20 dark:text-red-400"}`}
        >
  ...
  const renderActions = (user: User) => {
    const roleName =
      (typeof user.role_id === "object" ? user.role_id?.name : user.role) ||
      "—";
        ...
        {canEdit && (
          <Can permission="update.users">
            <Button
              ...
              size="icon"
              onClick={() =>
                router.push(`${ROUTES.ManageUsersAdd}?id=${user._id}`)
              }
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                variant="ghost"
                size="icon"
                className="w-10 h-10 border-none text-gray-500 hover:text-gray-600 hover:bg-gray-50 dark:hover:bg-(--table-hover) rounded-lg transition-all shadow-xs dark:bg-page-body"
              >
            ...
            <DropdownMenuContent align="end" className="w-60">
              ...
              {canImpersonate && (
                <DropdownMenuItem onClick={() => handleImpersonate(user)}>
                  <UserCheck
                    className={`w-4 h-4 mr-2 text-amber-600 ${isBeingImpersonated ? "animate-pulse" : ""}`}
                  />
                  <span>
                    {t("common_impersonate", {
                      defaultValue: "Login With Tenant (Impersonate)",
                    })}
                  </span>
                </DropdownMenuItem>
              )}
  ...
  return (
    <>
      ...
      <ConfirmModal
        ...
        subtitle={t("subscription_reset_limits_subtitle", {
      ...
      <ConfirmModal
        ...
        title={t("common_send_reset_link", {
          defaultValue: "Send Reset Password Link",
        })}
        subtitle={t("common_reset_password_subtitle", {
          defaultValue: `Are you sure you want to send a reset password link to ${selectedUser?.email}?`,
      ...
      <ConfirmModal
        ...
        subtitle={t("common_delete_user_subtitle", {
          defaultValue: `Are you sure you want to delete ${selectedUser?.name}? This action cannot be undone.`,
          name: selectedUser?.name,
        })}
