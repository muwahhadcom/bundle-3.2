...
import { tabs } from "@/src/data/landingPage";
...
import BrandingForm from "./sections/BrandingForm";
...
const LandingPageContainer = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (
        payload.platform_section &&
        Array.isArray(payload.platform_section.items)
      ) {
        payload.platform_section.items = payload.platform_section.items.map(
  ...
  const renderActiveTab = () => {
    ...
    switch (activeTab) {
      case "header":
        return (
          <HeaderForm
            data={formData.header_section || { menu_items: [] }}
                ...
                {tabs.map((tab) => {
                  ...
                  return (
                    <Button
                      ...
                      className={`
                        flex items-center h-[48px] [@media(min-width:1421px)]:h-[72px] gap-2 [@media(min-width:1421px)]:gap-4 px-3 [@media(min-width:1421px)]:px-5 py-2 [@media(min-width:1421px)]:py-4 rounded-lg text-left group relative overflow-hidden shrink-0 snap-center [@media(min-width:1421px)]:snap-start
              ...
              <div>
                <div className="flex items-center gap-2 mb-0.5 lg:mb-1">
                  ...
                  <h2 className="text-xl font-medium text-gray-900 dark:text-gray-100 tracking-tight">
                    {t(`landing_page_tabs_${activeTab}`)}{" "}
                    {t("landing_page_configuration")}
                  </h2>
                </div>
                <p className="text-[14px] text-gray-400 font-medium px-1">
                  {t(`landing_page_tabs_${activeTab}_desc`)}{" "}
                  {t("landing_page_global_appearance")}
                </p>
              </div>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2 lg:gap-3 relative ">
              ...
              {formData && (
                <div className="flex items-center gap-3 px-4 py-3 bg-gray-50 dark:bg-page-body rounded-lg border border-gray-100 dark:border-none shadow-inner">
                  <Label
                    ...
                    className="text-[13px] font-bold text-gray-600 dark:text-gray-300 cursor-pointer"
                  >
              ...
              <div className="flex-1 flex items-center justify-end gap-2 w-full">
                <Button
                  ...
                  size="sm"
                  onClick={refetch}
                  className=" [@media(min-width:1421px)]:flex-initial h-10 lg:h-12 px-4.5 py-5 gap-2 border-gray-100 dark:bg-page-body dark:border-none rounded-lg font-bold text-gray-500 dark:hover:bg-(--table-hover) hover:bg-gray-50 group"
                >
                  <RefreshCw
                ...
                <Can permission="update.landing_page">
                  <Button
                    ...
                    className=" [@media(min-width:1421px)]:flex-initial h-10 lg:h-12 px-4.5 py-5 gap-2 lg:gap-3 bg-primary hover:bg-primary/90 text-white shadow-2xl shadow-primary/30 rounded-lg font-black text-[11px] lg:text-[13px] transition-all active:scale-95 group"
                  >
                    {isUpdating ? (
            ...
            {isLoading ? (
              <div className="h-80 [@media(min-width:1421px)]:h-150 flex flex-col items-center justify-center bg-white dark:bg-(--card-color) rounded-lg [@media(min-width:1421px)]:rounded-lg border border-gray-100 dark:border-(--card-border-color) shadow-xl shadow-gray-100/50">
                ...
                <p className="text-[14px] lg:text-[15px] font-bold text-gray-900">
                  {t("landing_page_synchronizing")}
                </p>
                <p className="text-[11px] lg:text-[12px] text-gray-400 mt-1">
                  {t("landing_page_fetching_settings")}
                </p>
              </div>
            ) : (
