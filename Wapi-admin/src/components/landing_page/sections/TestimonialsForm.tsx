...
import { TestimonialsFormProps } from "@/src/types/landingPage";
import {
  Check,
  ChevronsUpDown,
  MessageSquareQuote,
...
const TestimonialsForm = ({ data, onChange }: TestimonialsFormProps) => {
  ...
  const toggleTestimonial = (id: string) => {
    const isSelected = selectedTestimonials.some(
      (t: any) => getObjId(t) === id,
    );
    if (isSelected) {
      handleChange(
      ...
      <div className="grid grid-cols-1 [@media(min-width:1421px)]:grid-cols-2 gap-10">
        ...
        <div className="flex flex-col justify-center border-l-2 border-primary/5 pl-10 hidden [@media(min-width:1421px)]:flex">
          <p className="text-[13px] text-gray-400 leading-relaxed font-medium">
            Build trust by showcasing authentic feedback from your power users.
            Select the most impactful testimonials from your library.
          </p>
        </div>
      </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[12px] text-gray-400">
                Search and pin customer feedback to the public wall
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-8">
          <Popover open={open} onOpenChange={setOpen}>
            <PopoverTrigger asChild>
              <Button
                ...
                role="combobox"
                aria-expanded={open}
                className="w-full [@media(min-width:1421px)]:w-[400px] justify-between bg-white dark:bg-page-body dark:hover:bg-page-body border-gray-200 dark:border-(--card-border-color) h-12 rounded-lg px-5 text-gray-500 font-medium shadow-sm"
              >
                <div className="flex items-center gap-2">
                  <Search className="w-4 h-4 opacity-40" />
                  <span>
                    {selectedTestimonials.length > 0
                      ? `${selectedTestimonials.length} stories curated`
                      : "Find testimonials..."}
                  </span>
                </div>
            ...
            <PopoverContent
              className="w-[var(--radix-popover-trigger-width)] [@media(min-width:1421px)]:w-[500px] p-0 rounded-lg border-gray-100 dark:border-(--card-border-color) shadow-2xl overflow-hidden"
              align="start"
            >
                ...
                <CommandList className="p-1 custom-scrollbar">
                  ...
                  <CommandGroup className="custom-scrollbar">
                    {testimonialsData?.data?.testimonials.map((testimonial) => {
                      const isSelected = selectedTestimonials.some((t: any) =>
                        typeof t === "string"
                          ? t === testimonial._id
                      ...
                      return (
                        <CommandItem
                          key={testimonial._id}
                          onSelect={() => toggleTestimonial(testimonial._id)}
                          className="rounded-lg dark:hover:bg-(--table-hover) flex items-center justify-between p-3 mb-1 cursor-pointer"
                        >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6 px-1">
            {testimonialsData?.data?.testimonials
              .filter((t) =>
                selectedTestimonials.some((st: any) => getObjId(st) === t._id),
              )
              .map((testimonial) => (
                <div
                  key={testimonial._id}
                  className="p-4 sm:p-6 bg-white dark:bg-page-body border border-gray-100 dark:border-(--card-border-color) rounded-lg  shadow-sm dark:border-none transition-all duration-300 relative group animate-in fade-in zoom-in-95 duration-500"
                >
                  <Button
                  ...
                  <div className="flex items-center gap-3 md:gap-4 mb-5">
                    <div className="w-12 h-12 md:w-14 md:h-14 rounded-xl md:rounded-[1.2rem] bg-primary/3 dark:bg-primary/[0.01] border border-primary/10 flex items-center justify-center shrink-0">
                      <span className="text-lg md:text-xl font-black text-primary/40">
                        {testimonial.user_name.charAt(0)}
                      </span>
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-[15px] md:text-[16px] font-bold text-gray-900 dark:text-gray-100 truncate">
                        {testimonial.user_name}
                      </p>
            ...
            {selectedTestimonials.length === 0 && (
              <div className="col-span-full py-20 border border-dashed border-gray-100 dark:border-(--card-border-color) rounded-lg flex flex-col items-center justify-center bg-gray-50/10 text-gray-400 dark:bg-(--page-body-bg)">
                ...
                <p className="text-[15px] font-semibold">
                  Your social proof wall is currently empty
                </p>
                <p className="text-[12px] opacity-60">
                  Add some customer testimonials to start building credibility
                </p>
              </div>
            )}
          </div>
