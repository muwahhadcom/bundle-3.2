...
const PlatformForm = ({ data, onChange }: PlatformFormProps) => {
  ...
  const removeItem = (index: number) => {
    ...
    // Re-index steps
    const reindexedItems = filteredItems.map((item, i) => ({
      ...item,
      step: i + 1,
    }));
      ...
      <div className="space-y-6 pt-10 border-t border-gray-100 dark:border-(--card-border-color)">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[12px] text-gray-400">
                Add detailed platform steps to guide your users
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-6 md:gap-8">
          {data.items.map((item, itemIndex) => (
            <div
              key={itemIndex}
              className="group relative flex flex-col xl:flex-row gap-6 md:gap-8 p-5 md:p-8 bg-white border border-gray-100 dark:border-(--card-border-color) rounded-lg dark:bg-(--dark-body) dark:border-none shadow-sm transition-all duration-300"
            >
              ...
              <div className="flex-1 grid grid-cols-1 [@media(min-width:1421px)]:grid-cols-12 gap-8 xl:pl-4">
                ...
                <div className="[@media(min-width:1421px)]:col-span-4 xl:col-span-4 space-y-6">
                  <ImageSelector
                    ...
                    value={item.image}
                    onChange={(url) =>
                      handleItemChange(itemIndex, "image", url)
                    }
                    className="space-y-2! md:max-w-md xl:max-w-none mx-auto lg:mx-0"
                  />

                  <div className="grid grid-cols-2 gap-4 [@media(max-width:475px)]:grid-cols-1">
                    ...
                    <div className="space-y-2 px-1">
                      ...
                      <Input
                        value={item.tagline}
                        onChange={(e) =>
                          handleItemChange(itemIndex, "tagline", e.target.value)
                        }
                        placeholder="TAGLINE"
                        className="h-10 text-xs bg-(--input-color) border-(--input-border-color) dark:bg-page-body rounded-lg"
                      />
                    </div>
                  </div>
                </div>
                  ...
                  <div className="space-y-3">
                    <div className="space-y-2 flex flex-col">
                      ...
                      <Input
                        ...
                        className="bg-(--input-color) border-(--input-border-color) dark:bg-page-body h-11 text-[15px] font-bold rounded-lg"
                      />
                    </div>
                    <div className="space-y-2 flex flex-col">
                      <Label className="text-[13px] font-bold text-gray-700 dark:text-gray-300">
                      ...
                      <Textarea
                        ...
                        placeholder="Explain how this step benefits the user..."
                        rows={2}
                        className="bg-(--input-color) dark:bg-page-body border-(--input-border-color) dark:border-(--card-border-color) resize-none text-[14px] leading-relaxed p-4 rounded-lg"
                      />
                    </div>
                  </div>
                  ...
                  <div className="space-y-4 bg-gray-50/30 dark:bg-(--card-color) sm:p-5 p-4 rounded-lg border border-gray-100 dark:border-none">
                    <div className="flex items-center justify-between px-1 flex-wrap gap-2 sm:gap-0">
                      ...
                      <Button
                        ...
                        className="h-8 text-[12px] bg-primary hover:bg-primary hover:text-white font-bold text-white rounded-lg px-2"
                      >
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {item.bullets.map((bullet, bulletIndex) => (
                        ...
                        >
                          <Input
                            ...
                            className="h-10 text-[13px] pr-12! bg-(--input-color) border-(--input-border-color) dark:bg-page-body shadow-sm rounded-lg pl-4 pr-10"
                          />
                          <Button
                            onClick={() => removeBullet(itemIndex, bulletIndex)}
                            className="absolute w-9 bg-[unset]! right-2 top-1/2 -translate-y-1/2 p-1.5 text-gray-300 hover:text-red-500 opacity-0 group-hover/bullet:opacity-100 transition-all hover:bg-red-50 dark:hover:bg-red-900/10 rounded-lg"
                          >
                      ...
                      {item.bullets.length === 0 && (
                        <div className="col-span-full py-4 text-center border border-dashed border-gray-200 dark:border-gray-800 rounded-lg text-gray-400 text-[11px] font-medium">
                ...
                <div className="absolute top-4 right-4 md:top-8 md:right-8 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                  <Button
                    ...
                    className="w-10 h-10 text-slate-400 hover:text-red-500 dark:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-all duration-300 bg-white/80 dark:bg-zinc-800/10 md:bg-transparent shadow-sm md:shadow-none"
                  >
          ...
          {data.items.length === 0 && (
            <div className="py-32 border-2 border-dashed border-gray-100 dark:border-(--card-border-color) rounded-[3.5rem] flex flex-col items-center justify-center text-gray-300 bg-gray-50/20">
              ...
              <p className="text-[16px] font-bold text-gray-400">
                Your platform story starts here
              </p>
              <Button
                onClick={addItem}
                variant="link"
          ...
          <div className="flex items-center justify-end">
            <Button
              onClick={addItem}
              size="sm"
              className="gap-2 h-10 px-6 rounded-lg bg-primary text-white hover:bg-primary/90 shadow-lg shadow-primary/20"
            >
