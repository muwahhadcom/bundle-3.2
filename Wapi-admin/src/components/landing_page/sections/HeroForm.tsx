...
import { HeroSection, FloatingImage, HeroFormProps } from "@/src/types/landingPage";
...
const HeroForm = ({ data, onChange }: HeroFormProps) => {
  ...
  const addFloatingImage = () => {
    handleChange("floating_images", [...data.floating_images, { url: "", position: "left" }]);
  };
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-400">Small UI elements to enhance landing page visual depth</p>
            </div>
          </div>
          {data.floating_images.length <= 1 && (
            ...
            <div key={index} className="group relative p-4 bg-gray-50/50 dark:bg-page-body border border-gray-100 dark:border-(--card-border-color) rounded-xl hover:bg-white dark:hover:bg-(--dark-body) transition-all duration-300 shadow-sm hover:shadow-md">
              ...
              <div className="space-y-4">
                ...
                <div className="px-1 space-y-2">
                  ...
                  <Select value={img.position?.includes("left") ? "left" : "right"} onValueChange={(val) => handleFloatingImageChange(index, "position", val as any)}>
                    ...
                    <SelectContent className="rounded-lg border-gray-100 dark:bg-(--card-color) dark:border-(--card-border-color) shadow-2xl">
                      <SelectItem value="left" className="rounded-lg dark:hover:bg-(--table-hover)">
                        Left
                      </SelectItem>
                      <SelectItem value="right" className="rounded-lg dark:hover:bg-(--table-hover)">
                        Right
                      </SelectItem>
                    </SelectContent>
                  </Select>
