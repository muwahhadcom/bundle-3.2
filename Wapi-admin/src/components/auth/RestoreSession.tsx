...
import { setCredentials, logout } from "@/src/redux/reducers/authSlice";
...
export default function RestoreSession() {
  ...
  useEffect(() => {
    ...
    const restoreSession = async () => {
      try {
        // Clear any existing stale state
        dispatch(logout());
        ...
        if (profileData.success && profileData.user) {
          ...
          // Separate flow: Redirect to Dashboard for self-tenant, ManageUsers for impersonation
          if (type === "self-tenant") {
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-slate-50 dark:bg-slate-900">
      <div className="text-center">
        ...
        <h1 className="text-xl font-semibold text-slate-900 dark:text-slate-100">{error ? "Session Restore Failed" : "Restoring Admin Session..."}</h1>
        <p className="mt-2 text-slate-500 dark:text-slate-400">{error || "Please wait while we log you back in"}</p>
      </div>
    </div>
  );
}
