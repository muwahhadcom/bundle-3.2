...
export const useAdminTemplateForm = (templateId?: string) => {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.marketing_type === "catalog") {
      const payload = { template_name: formData.template_name, language: formData.language, category: formData.category, message_body: messageBody, header_text: "", footer_text: "", variable_examples: formData.variables_example, buttons: [{ type: "catalog", text: formData.catalog_button_text || "View catalog" }], platform: formData.platform || "whatsapp", ...sectorPayload };
      try {
        if (templateId) await updateTemplate({ id: templateId, data: payload }).unwrap();
        else await createTemplate(payload).unwrap();
        toast.success("Template saved successfully");
    ...
    if (formData.marketing_type === "call_permission") {
      const payload = { template_name: formData.template_name, language: formData.language, category: formData.category, message_body: messageBody, header_text: "", footer_text: "", variable_examples: formData.variables_example, call_permission: "true", platform: formData.platform || "whatsapp", ...sectorPayload };
      try {
        if (templateId) await updateTemplate({ id: templateId, data: payload }).unwrap();
        else await createTemplate(payload).unwrap();
        toast.success("Template saved successfully");
    ...
    if (formData.marketing_type === "carousel_media") {
      ...
      fd.append("platform", formData.platform || "whatsapp");
    ...
    if (hasFile) {
      ...
      payload.append("platform", formData.platform || "whatsapp");
