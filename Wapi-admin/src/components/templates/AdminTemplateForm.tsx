...
export const AdminTemplateForm = ({ templateId }: AdminTemplateFormProps) => {
  ...
  return (
    <form onSubmit={onSubmit} className="flex flex-col">
      ...
      <div className="flex flex-1 flex-col lg:flex-row gap-6 py-4 sm:py-6">
        <div className="flex-1 space-y-10 min-w-0 ">
          <BasicInfoSection
            ...
            setPlatform={(v) =>
              setFormData((p: any) => {
                const updates: any = { platform: v };
                if (v && v !== "whatsapp") {
                  if (p.category === "AUTHENTICATION") {
                    updates.category = "UTILITY";
                  }
                  if (
                    p.category === "MARKETING" &&
                    !["none", "coupon_code", "carousel_media"].includes(p.marketing_type)
                  ) {
                    updates.marketing_type = "none";
                    updates.interactive_type = "none";
                    updates.buttons = [];
                  }
                }
                return { ...p, ...updates };
