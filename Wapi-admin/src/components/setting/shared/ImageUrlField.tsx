        ...
        <div className="flex gap-2 flex-wrap">
          <div className="relative flex-1">
            ...
            <Input
              ...
              autoFocus
              className="h-10 p-3 bg-(--input-color) dark:bg-page-body border-(--input-border-color) text-sm"
            />
          </div>
          <Button
            size="sm"
            onClick={handleConfirmUrl}
            className="h-10 px-4.5 py-5 bg-primary hover:bg-primary/90 text-white text-xs"
          >
            Apply
          </Button>
          <Button
            ...
            className="h-10 px-4.5 py-4 text-xs border-(--input-border-color) dark:bg-page-body dark:border-none dark:hover:bg-(--table-hover)  "
          >
            Cancel
          </Button>
        </div>
        ...
        <div className="group relative flex items-center gap-3 p-3 bg-gray-50 dark:bg-page-body border border-(--input-border-color) dark:border-(--card-border-color) rounded-lg">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-1.5">
              ...
              {hasLocalFile && (
                <span className="shrink-0 text-[10px] font-medium px-1.5 py-0.5 bg-amber-100 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400 rounded">
                  Pending upload
                </span>
              )}
            </div>
            <p className="text-xs text-gray-400 dark:text-gray-500 truncate">
              {hasLocalFile ? "Will be uploaded on save" : value}
            </p>
          </div>
          <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
            {!hasLocalFile && (
              <a
                ...
                className="p-1.5 rounded-lg hover:bg-gray-200 dark:hover:bg-(--table-hover) text-gray-500 transition-colors"
              >
            ...
            <Button
              title="Edit URL"
              onClick={handleEdit}
              className="p-1.5! bg-[unset]! h-[unset]! shadow-[unset]! rounded-lg hover:bg-gray-200 dark:hover:bg-(--table-hover) text-gray-500 transition-colors"
            >
            ...
            <Button
              title="Remove"
              onClick={handleClear}
              className="p-1.5! bg-[unset]! h-[unset]! shadow-[unset]! rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20 text-gray-400 hover:text-red-500 transition-colors"
            >
