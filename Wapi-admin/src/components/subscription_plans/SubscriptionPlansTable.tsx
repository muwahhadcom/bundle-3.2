...
const SubscriptionPlansTable = ({ subscriptions, page, totalPages, total, onPageChange, isLoading, limit, onLimitChange, onSortChange, visibleColumns, searchTerm, isFilterActive }: SubscriptionPlansTableProps & { onSortChange?: (sortBy: string, sortOrder: "asc" | "desc") => void; visibleColumns?: string[] }) => {
  ...
  const handleDownloadReceipt = async (receiptPath: string, subId: string, transactionId?: string) => {
    try {
      const url = getResolvedImageUrl(receiptPath);
      const fileExtension = receiptPath.split('.').pop() || "png";
      const filename = `receipt-${transactionId || subId}.${fileExtension}`;
      ...
      window.open(getResolvedImageUrl(receiptPath), "_blank");
    }
  };
  ...
  const renderActions = (sub: Subscription) => {
    ...
    const hasReceipt = sub.transaction_receipt && sub.transaction_receipt !== "null" && sub.transaction_receipt.trim() !== "";
    ...
    return (
      <div className="flex items-center gap-1.5">
        {isManualOrAdmin && isPending ? (
          ...
          !hasReceipt && <span className="text-gray-500 dark:text-gray-600 text-center ml-4">-</span>
        )}
        {hasReceipt && (
          <Button
            variant="ghost"
            size="icon"
            className="w-10 h-10 border-none text-blue-600 hover:text-blue-700 hover:bg-blue-50 rounded-lg dark:hover:bg-blue-900/20 transition-all shadow-xs dark:bg-page-body"
            onClick={() => handleDownloadReceipt(sub.transaction_receipt!, sub._id, sub.transaction_id)}
